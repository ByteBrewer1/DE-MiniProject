  ...
  for (var d = coords[2]; d < sizes[2] + coords[2] && no0val; d++) {
    for (var w = coords[0]; w < sizes[0] + coords[0] && no0val; w++) {
      for (var h = coords[1]; h < sizes[1] + coords[1] && no0val; h++) {
        ...
        has1val =
          has1val || KMap[d][w % KMap.Width][h % KMap.Height].Value == 1;
      }
    }
  }
...
function getCoverList(nCubeArray) {
  ...
  for (var i = 0; i < nCubeArray.length; i++) {
    for (var j = 0; j < nCubeArray[i].length; j++) {
      if (!alreadyCovered(nCubeArray[i][j], cover))
        cover.push(nCubeArray[i][j]);
    }
  }
  return cover;
}
  ...
  for (var d = 0; d < KMap.nLevels; d++) {
    for (var w = 0; w < KMap.Width; w++) {
      for (var h = 0; h < KMap.Height; h++) {
        if (KMap[d][w][h].Value == 1 && !alreadyCovered([w, h, d], cover))
          check = false;
      }
    }
  }
  ...
  while (d < KMap.nLevels) {
    ...
    while (h < KMap.Height) {
      while (w < KMap.Width) {
        ...
        if (KMap.Width == 4 && w === 0) {
          if (checkCube([w, h, d], [4, 1, 1]))
            newCubeSet.push(makeCube([w, h, d], [4, 1, 1]));
          if (checkCube([w, h, d], [4, 2, 1]))
            newCubeSet.push(makeCube([w, h, d], [4, 2, 1]));
        }
        ...
        if (KMap.Height == 4 && h === 0) {
          if (checkCube([w, h, d], [1, 4, 1]))
            newCubeSet.push(makeCube([w, h, d], [1, 4, 1]));
          if (checkCube([w, h, d], [2, 4, 1]))
            newCubeSet.push(makeCube([w, h, d], [2, 4, 1]));
        }
        newCubeSet = checkForCollisions(newCubeSet);
        ...
        if (w === 0 && h === 0 && KMap.Width == 4 && KMap.Height == 4) {
          if (checkCube([w, h, d], [4, 4, 1]))
            newCubeSet.push(makeCube([w, h, d], [4, 4, 1]));
        }
        newCubeSet = checkForCollisions(newCubeSet);

        for (var i = 0; i < newCubeSet.length; i++) {
          nCubeList.push(newCubeSet[i]);
        }
        w += 1;
      }
      w = 0;
    ...
    if (d === 0 && KMap.nLevels == 2) {
      while (h < KMap.Height) {
        while (w < KMap.Width) {
          ...
          //In case a cube takes the whole width.
          if (KMap.Width == 4 && w === 0) {
            if (checkCube([w, h, d], [4, 1, 2]))
              newCubeSet.push(makeCube([w, h, d], [4, 1, 2]));
            if (checkCube([w, h, d], [4, 2, 2]))
              newCubeSet.push(makeCube([w, h, d], [4, 2, 2]));
          }
          newCubeSet = checkForCollisions(newCubeSet);
          ...
          //In case a cube takes the whole height.
          if (KMap.Width == 4 && w === 0) {
            if (checkCube([w, h, d], [1, 4, 2]))
              newCubeSet.push(makeCube([w, h, d], [1, 4, 2]));
            if (checkCube([w, h, d], [2, 4, 2]))
              newCubeSet.push(makeCube([w, h, d], [2, 4, 2]));
          }
          newCubeSet = checkForCollisions(newCubeSet);
          ...
          //In case a cube takes the whole width and height.
          if (w === 0 && h === 0 && KMap.Width == 4 && KMap.Height == 4) {
            if (checkCube([w, h, d], [4, 4, 2]))
              newCubeSet.push(makeCube([w, h, d], [4, 4, 2]));
          }
          newCubeSet = checkForCollisions(newCubeSet);

          for (var j = 0; j < newCubeSet.length; j++) {
            nCubeList.push(newCubeSet[j]);
          }
          w += 1;
        }
        w = 0;
  ...
  for (var d = 0; d < KMap.nLevels; d++) {
    ...
    for (var w = 0; w < KMap.Width; w++) {
      ...
      for (var h = 0; h < KMap.Height; h++) {
        ...
        valueStr =
          toBinString(bitOrd[d], KMap.nLevels - 1) +
          toBinString(bitOrd[w], KMap.nVarX) +
          toBinString(bitOrd[h], KMap.nVarY);
  ...
  for (d = 0; d < KMap.nLevels; d++) {
    for (h = 0; h < KMap.Height; h++) {
      for (w = 0; w < KMap.Width; w++) {
        document.getElementById(KMap[d][w][h].Button_id).style.backgroundColor =
          normalColor;
      }
    }
  }
...
function setColor(nCube, color) {
  for (var i = 0; i < nCube.length; i++) {
    document.getElementById(
      KMap[nCube[i][2]][nCube[i][0]][nCube[i][1]].Button_id
...
function generateKMapHTML() {
  var text = "<center>Mentor : Harshul Yagnik Sir, Dr.Nilay Patel</center>";
  text +=
    "\n<center>values can be switched between '0' (false), '1' (true), and '-' (don't care).</center>";
  text += "<center></center><br /><center>";
  ...
  //Width of the matrix
  text +=
    "<tr><th></th><th></th><th colspan=" + KMap.Width * KMap.Height + 2 + ">";
  ...
  for (d = 0; d < KMap.nLevels; d++) {
    for (w = 0; w < KMap.Width; w++) {
      ...
      text +=
        "<th>" +
        toBinString(bitOrd[d * 4 + w], KMap.nVarX + KMap.nLevels - 1) +
        "</th>";
    }
  }
  text += "</tr>";
    ...
    for (d = 0; d < KMap.nLevels; d++) {
      for (w = 0; w < KMap.Width; w++) {
        ...
        text +=
          "<td  ID='" +
          KMap[d][w][h].TFD_id +
          "'; style='background-color:0xFF'>";
        text +=
          "<input ID=" +
          KMap[d][w][h].Button_id +
          " name=" +
          KMap[d][w][h].Button_id;
        text +=
          " type='button'  style='height:6mm;width:8mm' value=' " +
          boolToBin(KMap[d][w][h].Value);
        text +=
          " '; onClick=modifyKMEntry(KMap[" +
          d +
          "][" +
          w +
          "][" +
...
function getFunctionHTML(nCube, cubeId) {
  var ref =
    toBinString(bitOrd[nCube[0][2]], KMap.nLevels - 1) +
    toBinString(bitOrd[nCube[0][0]], KMap.nVarX) +
    toBinString(bitOrd[nCube[0][1]], KMap.nVarY);
  var logicFunct = [];
  ...
  if (nCube.length >= 2) {
    ...
    for (var i = 1; i < nCube.length; i++) {
      ref =
        toBinString(bitOrd[nCube[i][2]], KMap.nLevels - 1) +
        toBinString(bitOrd[nCube[i][0]], KMap.nVarX) +
        toBinString(bitOrd[nCube[i][1]], KMap.nVarY);
  ...
  for (var k = 0; k < logicFunct.length; k++) {
    if (logicFunct[k] === 0) {
      wholeMap = false;
      funct +=
        "<span style='text-decoration: overline'>" + varNames[k] + "</span>";
...
function generateSolutionHTML() {
  ...
  for (var x = 0; x < KMap.nVarX + KMap.nVarY + KMap.nLevels - 1; x++) {
    text += varNames[x];
    if (x != KMap.nVarX + KMap.nVarY + KMap.nLevels - 2) text += ",";
  }
  text += ") = ";
  ...
  text += "</center></h2>";
  text +=
    "<center><small>PROTIP: Hover the mouse over each term in the equation to lighten up the corresponding n-cube.</small></center>";
...
function findExCoord(nCube, ex, coord) {
  ...
  for (var i = 0; i < nCube.length; i++) {
    switch (ex) {
      case 0:
        if (size > nCube[i][coord]) size = nCube[i][coord];
        break;
      case 1:
        if (size < nCube[i][coord]) size = nCube[i][coord];
        break;
...
function writeNCubes() {
  ...
  if (nCubeList.length !== 0) {
    for (var i = 0; i < nCubeList.length; i++) {
      goesOutW =
        nCubeList[i][0][0] == 3 && findExCoord(nCubeList[i], 0, 0) === 0;
      goesOutH =
        nCubeList[i][0][1] == 3 && findExCoord(nCubeList[i], 0, 1) === 0;
      for (var d = 0; d < 2; d++) {
        if (findExCoord(nCubeList[i], d, 2) == d) {
          ...
          code +=
            ...
            (4 * d + nCubeList[i][0][0]) +
            "," +
            (KMap.Height - 1 - findExCoord(nCubeList[i], 1, 1)) +
            ".1)";
          if (goesOutW && goesOutH) {
            ...
            code +=
              ...
              "}\\put(" +
              4 * d +
              ",0.1){\\dashbox{0.2}(0.8,0.8){}}\n";
            code +=
              "\\color{" +
              colors[i % 16] +
              "}\\put(" +
              (4 * d + 3) +
              ",3.1){\\dashbox{0.2}(0.8,0.8){}}\n";
            code +=
              "\\color{" +
              colors[i % 16] +
              "}\\put(" +
            ...
            code +=
              "{\\dashbox{0.2}(0.8," +
              (findExCoord(nCubeList[i], 1, 1) -
                findExCoord(nCubeList[i], 0, 1)) +
              ".8){}}\n";
            code +=
              "\\color{" +
              colors[i % 16] +
            ...
            code +=
              "{\\dashbox{0.2}(" +
              (findExCoord(nCubeList[i], 1, 0) -
                findExCoord(nCubeList[i], 0, 0)) +
              ".8,0.8){}}\n";
            code +=
              "\\color{" +
              colors[i % 16] +
